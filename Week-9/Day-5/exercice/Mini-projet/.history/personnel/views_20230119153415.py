...
from account.models import RoomManager
...
def dashboard(request):
  ...
  if request.session.get('username',None) and request.session.get('type',None)=='manager':
      ...
      print(booked)
      return render(request,"perso_dash/index.html",{"room_data":room_data,"manager":data,"booked":booked})
  else:
      return redirect("manager_login")
def add_room(request):
    ...
    if request.method=="GET":
        return render(request,"perso_dash/add-room.html",{})
    else:
...
def update_room(request,room_no):
    ...
    if request.method=="GET":
        return render(request,"perso_dash/edit-room.html",{"room":room})
    else:
            price=request.POST['price']
            no_of_days_advance=request.POST['no_of_days_advance']
            error=[]

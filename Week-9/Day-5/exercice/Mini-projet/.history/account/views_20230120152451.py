...
def user_login(request):
    ...
    if request.method=="POST":
        ...
        if not len(username):
            messages.warning(request,"Le champ username est vide")
            redirect('user_login')
        elif not len(password):
            messages.warning(request,"Le champ password est vide")
            redirect('user_login')
        else:
            pass
    ...
    if request.method=="POST":
        ...
        if Customer.objects.filter(username=username) or Customer.objects.filter(email=email):
           messages.warning(request,"Le compte exite déjà, svp veillez vous connectez pour continuer")
            ...
            if(len(username)<3):
                error.append(1)
                messages.warning(request,"Le champ Username Field doit dépassé 3 caractères.")
            if(len(password)<5):
                error.append(1)
            ...
            if(len(error)==0):
                ...
                customer.save()
                messages.info(request,"Votre compte a été crée avec succès!!! Successfully, veillez vous connectez pour contin")

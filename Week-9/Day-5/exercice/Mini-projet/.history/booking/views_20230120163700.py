...
def contact(request):
    if request.method=="GET":
     ...
     data.save()
     return render(request,"contact/contact.html",{'message':'Merci de nous avoir contacter.'})
def book(request):
...
def book_confirm(request):
    ...
    messages.info(request,"La chambre a été reservée avec succès")
...
def cancel_room(request,id):
    ...
    room.save()
    data.delete()
    return HttpResponse("Reservation annulée avec succès!!!")
...
def delete_room(request,id):
    ...
    if manager==request.session['username']:
        data.delete()
        return HttpResponse("Vous avez supprimer la chambre avec succès!!!")
    else:
        return HttpResponse("Requête inv")
